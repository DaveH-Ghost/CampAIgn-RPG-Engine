...
from campaign_rpg_engine.world_edit_api import WorldMutationResult, create_agent_in_area, create_object_in_area
...
def validate_template(data: dict[str, Any]) -> str | None:
    ...
    if kind == "agent":
        module_id = data.get("memory_module") or (
            (data.get("memory") or {}).get("module_id")
        )
        if not module_id:
