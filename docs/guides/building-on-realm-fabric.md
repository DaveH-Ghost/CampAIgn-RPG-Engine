...
Guide for application authors integrating **Realm-Fabric 1.0** as a library. Import only from **`realm_fabric`** in product code — the package is a single tree at `realm_fabric/` (no `src.*` layout).
...
Runnable template: [Realm-Studio](https://github.com/DaveH-Ghost/Realm-Studio) (`backend/` shows HTTP + typed `Session` usage).
...
```
my-realm-game/
  pyproject.toml          # depends on realm-fabric>=1.0.0
  my_game/
...
Keep **scenario content** (handlers, lorebooks, UI) in your repo. Use **Realm-Studio** as the HTTP + GM reference; fork it only if you need a full editor.
...
| GM authoring UI | Typed HTTP handlers (see Realm-Studio `area_api.py`) or your own forms |
| Legacy stepper strings | App-owned dispatch only (Realm-Studio `command_dispatch.py` imports `realm_fabric.area_edit` helpers) |
...
| `realm_fabric.area_edit` string helpers | Internal — for tests and app-owned GM dispatch |
