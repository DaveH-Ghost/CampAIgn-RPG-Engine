...
| **0.3.1b** | Grid UI — agents, objects, active-agent highlight | ⬜ Planned |
| **0.3.1c** | Right-click menus + modals → `run_command` / `set_active_agent` | ⬜ Planned |
...
- `examples/web/realm-studio/tests/test_api.py` — health, snapshot shape, index page (3 tests)
...
| In | Detail |
|----|--------|
| Grid render | Cells from `snapshot.grid` (dynamic bounds, default 5×5 demo) |
| Entity chips | Agents and objects at `position[]`; distinct styling (e.g. agent vs object) |
| Active agent highlight | Border/star from `snapshot.active_agent_id` |
| Stacked entities | Multiple chips per tile when positions collide |
| Coordinates | Show `(x, y)` on hover or tile corner (helps editing) |
| Refresh | Re-fetch `GET /api/state` on load; after mutations (called from 0.3.1c+) |
| Labels | Name on chip or tooltip (`name` + `id`) |

### Out

- Right-click menus ( **0.3.1c** )
- Sprites / `appearance` field (**0.3.2+**)
- Drag-and-drop move
...
| In | Detail |
|----|--------|
| **`POST /api/command`** | Body: `{ "line": "create-object ..." }` → `session.run_command` |
| **`POST /api/active-agent`** | Body: `{ "name_or_id": "Goblin" }` → `set_active_agent` |
| **Context menus** | Per [Right-click context menus](#right-click-context-menus-031c) above |
| **Modals** | Create/edit forms → assemble CLI command strings; position pre-filled from clicked tile |
| **Error display** | Toast or inline panel with `CommandResult.message` |
| **State refresh** | Successful command → re-fetch snapshot → grid redraw |
| **Toolbar active agent** | Dropdown synced with `active_agent_id` (optional duplicate of Play as) |

### CLI parity targets (v1)

- `create-object`, `create-agent`, `edit-object`, `edit-agent`, `delete-object`, `delete-agent`
- `set_active_agent` via API (not a turn)
- Read-only listings via snapshot refresh (no separate `list` command required in UI)

### Out

- Full CLI edit surface (memory modules, `add-action` / effects builder, …)
- Drag-and-drop reposition
- Auth / multi-user sessions
- In-browser `GameProfile` / prompt template editor
